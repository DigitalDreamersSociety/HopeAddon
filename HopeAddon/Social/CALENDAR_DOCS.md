...
**Source files:**
- `Calendar.lua` - Data & logic (~2168 lines)
- `CalendarUI.lua` - Rendering (~6460 lines)
- `CalendarValidation.lua` - Validation (~271 lines)
- Calendar constants in `Constants.lua` lines 8990-9519
...
### One-Time Server Events (`C.SERVER_EVENTS`, Constants.lua:9215-9267)

| ID | Title | Date | Time |
|----|-------|------|------|
| dark_portal_opening | The Dark Portal Opens | 2026-02-05 | 17:00 |
| test_server_2026 | Server Maintenance | 2026-02-04 | 07:00 |
...
| weekly_guild_hangout | Guild Hangout | Thursday (wday 5) | 18:00 | Teal `{0.2, 0.8, 0.8}` | `INV_Misc_QuestionMark` |
...
- `GetUpcomingEvents()` - generates instances for the next 30 days (~12-15 events)
...
| `GetPastEvents()` | 775 | Past events within configured days |
| `GetEventsForMonth(year, month)` | 844 | Full month grid data |
| `SignUp(eventId, role, status)` | 970 | Sign up with validation |
| `CancelSignup(eventId)` | 1091 | Remove signup |
| `GetSignupCounts(event)` | 1137 | {tank, healer, dps} counts |
| `GetSignupsByRole(event)` | 1176 | Grouped by role + status |
| `SaveTemplate(eventId, name)` | 1290 | Save event as template |
| `LoadTemplate(templateId)` | 1375 | Load template data |
| `CheckNotifications()` | 1447 | 1hr/15min reminder check |
| `CleanupExpiredEvents()` | 1512 | Remove events > 30 days old |
| `SerializeEvent(event)` | 1602 | Pipe-delimited for network |
| `DeserializeEvent(data)` | 1627 | Parse from network |
| `BroadcastEvent(event, action)` | 1656 | Send to guild/party |
| `LockEvent(eventId)` | 1813 | Lock signups |
| `UnlockEvent(eventId)` | 1838 | Unlock signups |
| `SetPlayerRosterStatus(...)` | 1879 | Set TEAM/ALTERNATE/DECLINED |
| `CyclePlayerRosterStatus(...)` | 1912 | Cycle through statuses |
| `GetRosterByStatus(event)` | 1951 | Get roster grouped by status |
| `ExportEventDetails(event)` | 2029 | Plain text export |

### CalendarUI.lua - Rendering

| Function | Line | Purpose |
|----------|------|---------|
| `CreatePools()` | 169 | dayCellPool + eventCardPool |
| `CreateDayCellFrame()` | 237 | Month grid cell (80x60px) |
| `CreateMiniEventCard(parent)` | 388 | Small event card in cell (60x14px) |
| `PopulateMiniCards(cell, events, dateStr)` | 627 | Fill cell with event cards |
| `ApplyServerEventTheme(cell, event)` | 777 | Themed background for server events |
| `CreateCalendarHeader(parent)` | 1715 | "RAID CALENDAR" + Create button |
| `CreateMonthNavigation(parent)` | 1772 | < Month Year > navigation |
| `NavigateMonth(delta)` | 1825 | Move month forward/back |
| `NavigateToDate(y, m, d)` | 1860 | Jump to specific date |
| `CreateMonthGrid(parent)` | 1896 | 7x6 grid layout |
| `PopulateMonthGrid(grid)` | 1932 | Two-pass: create cells, align heights |
| `CreateWeekCalendar(parent)` | 2088 | Continuous week view |
| `AcquireEventCard(event)` | 2304 | Full event card for popups |
| `GetEventDetailPopup()` | 2417 | 600x700px event detail |
| `ShowEventDetail(event)` | 2863 | Populate detail popup |
| `GetServerEventPopup()` | 3133 | Read-only server event popup |
| `GetUnifiedDayPopup()` | 3448 | Master-detail day view (900x600px) |
| `ShowUnifiedDayPopup(dateStr, selectedEventId, openCreateMode)` | 4919 | Main entry for day popup |
| `PopulateSignupList(content, event)` | 5125 | Signup matrix display |
| `PopulateRosterManagement(...)` | 5408 | Owner roster management |
...
- **Journey tab**: `Journal:CreateUpcomingEventsSection()` shows up to 8 upcoming events (including server events) with local time conversion via `Calendar:GetDualTimes()`
