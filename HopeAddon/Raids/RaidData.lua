...
-- (Raid stats window constants removed — replaced by KillFlash module)
...
--============================================================
-- RAID BOSS STATS (replaced by KillFlash module)
--============================================================

-- Stub: prevents errors from any stale references
function RaidData:CreateRaidStatsWindow() end

-- Redirect to KillFlash if called directly
function RaidData:ShowRaidBossStats(raidKey, bossId, killData)
    if HopeAddon.KillFlash then
        local encounterSummary = nil
        if HopeAddon.EncounterTracker then
            encounterSummary = HopeAddon.EncounterTracker:GetEncounterSummary()
        end
        HopeAddon.KillFlash:ShowFlash(raidKey, bossId, killData, encounterSummary)
    end
end

-- No-op stub, call callback if provided
...
function RaidData:OnCombatLogEvent(...)
    ...
    -- Persist player's combat stats (before EncounterTracker is cleared)
    if killData and HopeAddon.CombatRecords then
        HopeAddon.CombatRecords:RecordEncounter(mapping.raid, mapping.boss, killData)
    end

    if killData then
        -- Show kill flash overlay
        local encounterSummary = nil
        if HopeAddon.EncounterTracker then
            encounterSummary = HopeAddon.EncounterTracker:GetEncounterSummary()
        end
